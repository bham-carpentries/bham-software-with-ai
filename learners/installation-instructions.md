...
You will also need to [create a GitHub account](#github-account) if you do not have one already, make sure that you are able to log into it, and download 
the [inflammation data and analysis code](#inflammation) which we will be used for exercises in the course.
...
The recommended way to do that for this course is to set up [**SSH authentication**](https://www.ssh.com/academy/ssh/public-key-authentication) 
which requires a pair of keys - one public that you upload to your GitHub account, and one private that remains on your machine.
...
## Inflammation data and analysis code {#inflammation}

As part of the course, we will be using some example code 
and data available on GitHub, which we’ll clone onto our machines using the Bash shell. 

So firstly open a Bash shell (via Git Bash in Windows or Terminal on a Mac). 
Then, on the command line, navigate to where you’d like the example code to reside, and use Git to clone it. 
